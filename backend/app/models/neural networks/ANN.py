...
class ANN:
    def __init__(self, layers=[64, 32, 10], activation='relu', learningRate=0.01, 
    ...
    def fit(self, X, y):
        ...
        for epoch in range(self.maxEpochs):
            ...
            for i in range(0, nSamples, self.batchSize):
                ...
                yPred = self.forwardPropagation(XBatch)
                
                batchLoss = self.computeLoss(yPred, yBatch)
                epochLoss += batchLoss * len(XBatch)
                
                batchPreds = np.argmax(yPred, axis=1)
                correctPredictions += np.sum(batchPreds == yBatch)
                
                gradientsW, gradientsB = self.backwardPropagation(XBatch, yBatch)
